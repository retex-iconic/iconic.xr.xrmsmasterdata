...
public class NegozioCommandServiceImpl implements NegozioCommandService {
    ...
    public NegozioCommandServiceImpl(CommandGateway commandGateway, EventStore eventStore, EventSourcingRepository<NegozioAggregate> negozioAggregateEventSourcingRepository) {
    ...
    public CompletableFuture<Object> createNegozio(NegozioCreateDTO negozioCreateDTO) {
        CompletableFuture<Object> result = commandGateway.send(new NegozioCreateCommand(
            UUID.randomUUID().toString(),
            negozioCreateDTO.getCode()
        ));
        return result;
    }
    ...
    public NegozioAggregate getNegozioAggregate(String negozioId) {
        ...
        return negozioAggregateEventSourcingRepository.load(negozioId).getWrappedAggregate().getAggregateRoot();
    }

    @Override
    public CompletableFuture<String> articoloIndex(String negozioId, StoreArticoloIndexDTO storeArticoloIndexDTO) {
        return commandGateway.send(new StoreArticoloIndexCommand(negozioId+" --> ", storeArticoloIndexDTO.getArticoloId()));
    }
